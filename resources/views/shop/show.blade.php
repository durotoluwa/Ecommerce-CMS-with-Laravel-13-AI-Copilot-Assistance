...
<main class="main">
          	<div class="page-header text-center" style="background-image: url('assets/images/page-header-bg.jpg')">
        		<div class="container">
        		<h1 class="page-title">
    {{ isset($category) ? $category->name : (isset($brand) ? $brand->name : '') }}
</h1>
...
// Checkbox & price input handler
document.addEventListener('change', function(e) {
    const checkbox = e.target.closest('.filter-checkbox');
    const priceInput = e.target.closest('#min-price') || e.target.closest('#max-price');
    if (!checkbox && !priceInput) return;

    const filters = collectFilters();

    fetch("{{ route('category.filter', isset($category) ? $category->id : $brand->id) }}", {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.nextPage) {
                wrapper.innerHTML = `<button id="load-more" class="btn btn-primary" data-url="${data.nextPage}">Load More</button>`;
            } else {
                wrapper.innerHTML = '';
            }

            document.querySelector('.toolbox-info').innerHTML =
                `Showing <span>${data.firstItem}–${data.lastItem}</span> of ${data.total} Products`;
        }
    });
});
...
const currencySymbol = priceSlider.dataset.symbol;
...
noUiSlider.create(priceSlider, {
    ...
    tooltips: true,
    format: {
        to: value => `${currencySymbol}${Number(value).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`,
        from: value => Number(value.replace(currencySymbol, '').replace(/,/g, ''))
    }
});

const priceRange = document.getElementById('filter-price-range');
priceSlider.noUiSlider.on('update', function(values, handle, unformatted) {
    // Show formatted text
    priceRange.innerHTML = `${values[0]} – ${values[1]}`;

    // Store raw numeric values for backend
    document.getElementById('min-price').value = unformatted[0];
    document.getElementById('max-price').value = unformatted[1];
});
