 ...
 <div class="page-header">
						<div class="add-item d-flex">
							<div class="page-title">
								<h4 class="fw-bold">All Currency</h4>
        ...
        <tbody>
          @foreach($currencies as $currency)
                <tr>
                    ...
                    <td>{{ $currency->rate }}</td>
            <td>{{ $currency->is_base ? 'Yes' : 'No' }}</td>
<td>{{ $currency->is_display ? 'Yes' : 'No' }}</td>

                    <td>
                        <a href="{{ route('admin.currencies.edit', $currency->id) }}" class="btn btn-sm btn-warning">Edit</a>
                        <form action="{{ route('admin.currencies.destroy', $currency->id) }}" method="POST" style="display:inline;">
                            @csrf @method('DELETE')
