...
<div class="table-responsive product-list">
    ...
    <a href="#" class="product-img stock-img">
        <img src="{{ asset($post->featured_image) }}" width="30" height="30" alt="{{ $post->title }}">
    </a>
    <a href="#">{{ Str::limit($post->title, 22) }}</a>
</div>
            </td>

                <td>{{ $post->status }}</td>
           <td>{{ \Carbon\Carbon::parse($post->publish_date)->format('F j, Y g:i A') }}</td>


                <td>
                  
                    <a href="{{ route('pages.edit', $post->id) }}" class="btn btn-warning btn-sm">Edit</a>
                    <form action="{{ route('pages.destroy', $post->id) }}" method="POST" style="display:inline;">
                        @csrf @method('DELETE')
