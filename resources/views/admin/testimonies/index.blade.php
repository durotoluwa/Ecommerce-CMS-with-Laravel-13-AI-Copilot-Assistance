 ...
 <div class="page-header">
						<div class="add-item d-flex">
							<div class="page-title">
								<h4 class="fw-bold">All Testimonies</h4>
...
<div class="card">
<div class="card-body">
<a href="{{ route('testimonies.create') }}" class="btn btn-primary mb-3">Create New Testimony</a>
                    ...
                    <div class="productimgname">
                        <a href="#" class="product-img stock-img">
                            @if($testimony->customer_image)
                                <img src="{{ asset($testimony->customer_image) }}" width="30" height="30" alt="{{ $testimony->customer_name }}">
                            @else
                                <img src="{{ asset('images/default-avatar.png') }}" width="30" height="30" alt="Default">
                            @endif
                        </a>
                        <a href="#">{{ Str::limit($testimony->customer_name, 22) }}</a>
                    </div>
                </td>

                <td>{{ $testimony->title ?? '—' }}</td>
                <td>{{ $testimony->status ?? 'Draft' }}</td>
                <td>{{ \Carbon\Carbon::parse($testimony->created_at)->format('F j, Y g:i A') }}</td>

                <td>
                      <a class="me-2 p-2" href="{{ route('testimonies.edit', $testimony->id) }}">
    <i data-bs-toggle="tooltip" title="Edit Product" data-feather="edit"></i>
</a>                    <form action="{{ route('testimonies.destroy', $testimony->id) }}" method="POST" style="display:inline;">
                        @csrf @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-sm"
                            onclick="return confirm('Delete this testimony?')">    <i data-bs-toggle="tooltip" title="Delete Product" data-feather="trash-2"></i></button>
                    </form>
