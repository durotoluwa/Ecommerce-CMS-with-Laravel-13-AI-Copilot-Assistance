...
document.getElementById('placeOrderBtn').addEventListener('click', function(e) {
    ...
    if (method === 'paystack') {
        ...
        var handler = PaystackPop.setup({
            ...
            email: '{{ Auth::guard("customer")->check() ? Auth::guard("customer")->user()->email : "guest@example.com" }}',
            amount: {{ session('cart_total', 0) * 100 }},
            currency: "NGN",
            ref: ''+Math.floor((Math.random() * 1000000000) + 1),
            callback: function(response){
                // Redirect to callback route with reference
window.location.href = "{{ route('checkout.paystack.callback') }}?reference=" + response.reference;
            },
            onClose: function(){
