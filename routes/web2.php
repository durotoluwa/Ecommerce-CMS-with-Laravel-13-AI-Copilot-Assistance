...
Route::match(['get','post'], '/shop/filter/categories', [ShopController::class, 'filterByCategories'])
    ->name('shop.filter.categories');

Route::match(['get','post'], '/category/{id?}/filter', [ShopController::class, 'filterByCategoryPage'])
    ->name('category.filter');
    
Route::match(['get', 'post'], '/brand/{id?}/filter', [ShopController::class,'filterBybrandPage'])
   ->name('brand.filter');
...
//::get('/product/{id}', [ProductController::class, 'showdetails'])->name('product.details');

Route::get('/product/{slug}', [ProductController::class, 'showdetails'])->name('product.details');



Route::get('admin/products/sample-csv', [ProductController::class, 'downloadSampleCsv'])
    ->name('admin.products.sample.csv');
 
Route::post('/checkout/place-order', [CheckoutController::class, 'placeOrder'])
    ->name('checkout.placeOrder');

    Route::get('/product/{id}/quickview', [UserController::class, 'quickView'])
...
//Route::get('/checkout/paystack/callback', [CheckoutController::class, 'confirmPaystackPayment'])
  //  ->name('checkout.paystack.callback');
 
    Route::view('/checkout/paystack', 'checkout.paystack')->name('checkout.paystack');

    Route::get('/checkout/paystack/callback/{order}', [CheckoutController::class, 'paystackCallback'])
    ->name('paystack.callback');


 
    Route::post('/checkout/store-order', [CheckoutController::class, 'storeOrder'])
    ->middleware('auth:customer') // force login
...
Route::post('admin/staff/create', [AdminUserController::class, 'storeAdminUser'])
        ->name('createstaff');
Route::put('admin/users/{id}/password', [AdminUserController::class, 'adminchangePassword'])
    ->name('change_password');

Route::delete('admin/users/{id}', [AdminUserController::class, 'destroy'])
    ->name('delete_user');

    Route::put('admin/profile/{id}', [AdminUserController::class, 'updateProfile'])
    ->name('adminupdateprofile');

   Route::get('admin/admin_user/{id}', [AdminUserController::class, 'adminprofile'])
    ->name('admin.admin_user.myprofile');
...
Route::get('/menus/references/{type}', [MenuController::class, 'references'])
    ->name('menus.references');

    });


    Route::get('admin/footer/edit', [FooterController::class, 'edit'])
    ->name('admin.footer.edit');

// Handle update
Route::put('admin/footer/update', [FooterController::class, 'update'])
...
Route::prefix('admin')->middleware(['auth'])->group(function () {
    ...
    Route::post('menus/items/reorder', [\App\Http\Controllers\Admin\MenuItemController::class, 'reorder'])
...
Route::put('/admin/homepage/categorysection/{id}', [HomePageSectionController::class, 'updatecategorySection'])
    ->name('home-category-section.update');


Route::get('/homepage/sectionbanner', [HomePageSectionController::class, 'SectionBanner'])->name('admin.homepage.sectionbanner');

Route::put('/admin/homepage/sectionbanner/{id}', [HomePageSectionController::class, 'updateSectionBanner'])
    ->name('sectionbanner.update');


    Route::get('/homepage/actionbox', [HomePageSectionController::class, 'Actionbox'])->name('admin.homepage.actionbox');
Route::put('/admin/homepage/actionbox/{id}', [HomePageSectionController::class, 'updateActionbox'])
    ->name('actionbox.update');
...
Route::put('/admin/homepage/product_section_one/{id}', [HomePageSectionController::class, 'updateProductsecone'])
    ->name('product_section_one.update');
   

    Route::get('/homepage/sectionbanner2', [HomePageSectionController::class, 'cardboxPage'])->name('admin.homepage.sectionbanner2');
Route::put('/admin/homepage/sectionbanner2/{id}', [HomePageSectionController::class, 'updateCardBoxes'])
    ->name('cardbox.update');
   
    Route::get('/homepage/producttab', [HomePageSectionController::class, 'productTabPage'])->name('admin.homepage.producttab');


    Route::put('/admin/homepage/producttab/{id}', [HomePageSectionController::class, 'updateProductTabs'])
    ->name('producttab.update');
...
Route::get('/admin/dashboard', [DashboardController::class, 'adminDashboard'])->name('admin.dashboard');
  ...
  Route::get('/product_orders/pending_product', [ProductController::class, 'pendingOrder'])->name('admin.product_orders.pending_product');
    ...
    Route::get('/payment/paystack', [PaymentController::class, 'paysatckPage'])
        ->name('admin.payment.paystack');

    // Update Paystack settings
 

        Route::put('/paystack/update', [PaymentController::class, 'updatepaystack'])
    ->name('admin.paystack.update');
...
// ============ menu builder
Route::middleware(['permission:menu configuration'])->group(function () {
Route::get('/admin/menu/menu-builder', [MenuItemController::class, 'index'])->name('admin.menu.menu-builder');
...
// ============ product page
 ...
 Route::get('/product_orders', [AdminOrderController::class, 'index'])->name('admin.product_orders.index');
    Route::get('/product_orders/{id}', [AdminOrderController::class, 'show'])->name('admin.product_orders.show');
});


Route::post('/admin/attributes/{attribute}/terms',
    [AttributeTermController::class, 'storeAttributeItem']
);
